...
class AELPlant:
    ...
    def _solve_current_for_power(self, T: float, P_avail: float) -> float:
        ...
        Returns 0 if P_avail is below the minimum feasible power
        (electrolyzer shuts down — cannot sustain minimum current).
        """
    ...
    def step(self, u: np.ndarray, d: np.ndarray, dt: float) -> np.ndarray:
        ...
        if I > 0.0:
            ...
            # Electrolyzer shutdown: insufficient power
            V, P_el, n_h2, sec = 0.0, 0.0, 0.0, 0.0
